...
def compute_logical_azimuth_to_target(theta_self, theta_target):
    ...
    - Here we compute how much we must rotate from that "center-pointing"
      direction to point from OUR position to another turret.
    """
...
def auto_sequence(m_alt, m_az):
    """
    From calibrated (0,0), aim at each *other* turret and fire FIRE_TIME seconds.
    - m_alt: altitude motor (index 0)  [we leave it at logical 0]
    - m_az : azimuth motor (index 1)  [we rotate this in auto sequence]
    """
    ...
    try:
        ...
        # Altitude: keep at logical 0 (whatever you calibrated to hit center pole)
        logical_alt = 0.0
        phys_alt = physical_from_logical(logical_alt, calib_alt)
